...
void Update() {
    ...
    if(has_gui){
        ...
        while(callback != ""){
            PrintDebug("AS Callback: "+callback+"\n");
            if(callback == "retry"){
	...
	if(has_client_connect_gui || connected_to_server){
			...
			if(type_check == "type"){
				...
				for (int a = 0; a < first_level_size; a++){
					if(first_level[a][0][1] == "SignOn"){
						...
						PrintDebug("id! " + remote_player_id + " " + remote_player_name + "\n");
						
						if(remote_player_id != -1){
							MovementObject@ remote_player = remote_players[remote_player_id];
							PrintDebug("MovementObject did not crash\n");
		...
		if(connected_to_server){
			...
			while(chat_callback != ""){
				PrintDebug("chat callback:" + callback + "\n");
	...
	if(!has_gui && !has_client_connect_gui && GetInputPressed(controller_id, "f12") && GetPlayerCharacterID() != -1){
		//client_connect_id = gui.AddGUI("text2","script_text.html",400,200, _GG_IGNORES_MOUSE);
	...
	if(connected_to_server && GetInputPressed(controller_id, "return")){
		//PrintDebug("Showing chat" + "\n");
		has_chat_gui = !has_chat_gui;
		gui.Execute(chat_id,"toggleChat();");
	}

	if(GetInputDown(controller_id, "f11") && connected_to_server){
	...
	if(!connected_to_server && DebugKeysEnabled() && GetInputPressed(controller_id, "l")){
	...
	if(connected_to_server && GetInputPressed(controller_id, "k")){
		MovementObject@ char = ReadCharacter(0);
